...
fn main() {
    ...
    let temperature: f32 = match temperature.trim().parse() {
        Ok(t) => t,
        Err(_) => panic!("Please, only enter numbers")
    };
    ...
    println!("Your entry of {}{} is {}", temperature, unit, convert(temperature, &unit));
}

fn convert(temp: f32, unit: &str) -> f32 {
    let unit = unit.to_lowercase();
    if unit == "c" { return (temp * 9.0 / 5.0) + 32.0 }
    if unit == "f" { return (temp - 32.0) * 5.0 / 9.0 }
    panic!("invalid unit");
}
